...
class Board extends React.Component {
	...
	handleChangename(event){
		this.setState({name : event.target.value, a: this.state.a, b: this.state.b, val: this.state.val})
	}

	handleChangeA(event) {
		this.setState({name : this.state.name, a: event.target.value, b: this.state.b, val: this.state.val});
	}

	handleChangeB(event) {
		this.setState({name : this.state.name, a: this.state.a, b: event.target.value, val: this.state.val});
	}

	handleChangeC(event) {
		this.setState({name : this.state.name, a: this.state.a, b: this.state.b, val: event.target.value});
	}
	...
	renderSquare(i, j) {
		if(j == 0) {
			++ i;
			return <Square
				value = {this.state.name + " " + i} color = {2}
			/>;
		}
		let X = 0; 
		...
		return <Square
			value = {this.props.state.board[i + 1][j]} color = {X}
			/>;
	}
	...
	renderName(x) {
		return <Square
			value = {this.state.name + " " + x} color = {2} />
	}
	...
	render() {
		...
		let status = [];
		if (isEnded === null) status.push('You have used ' + this.props.state.step + ((this.props.step >= 2) ? ' moves!' : ' move!')); 
		else if (isEnded == "won") {
			...
			status.push(<div className="star">⋆⋆</div>);
			status.push(<div style={{color : "yellowgreen"}}>You have a good solution</div>);
		}
		...
		<label class="label"> NAME </label>
			<input type="text" value={this.state.name} onChange={this.handleChangename} maxLength='12' />
		...
		<label class="label">
          Balance:
          <input type="number" Min = "1" Max = "100000" value={this.state.val} onChange={this.handleChangeC} />
        </label>
