...
function generateState(N){
  ...
  let bit = [...Array(1 << N)].fill(0), sum = [];

  for (let i = N; i > 0; i--) for (let j = 1; j < i; j++)
      board[i][j] = Math.floor(Math.random() * 200) - 100, board[j][i] = -board[i][j];
  ...
  for (let i = column+1; i <= N; i++) col[Math.floor(Math.random() * column)].push(i);
  ...
  for (let i = 0; i < column; i++){
    ...
    while (num--){
      ...
      board[a][b] += c; sum[a-1] += c;
      board[b][a] -= c; sum[b-1] -= c;
    }
  }

  for (let mask = 1; mask < (1 << N); mask++){
    let S = 0; bit[mask] = -1000000000;
    for (let i = 0; i < N; i++){
      if (mask&(1 << i)) S += sum[i];
    }
    if (S === 0) bit[mask] = 1;
    for (let i = mask; i > 0; i = (i-1)&mask){
      if (bit[mask] < bit[i] + bit[i^mask]) bit[mask] = bit[i] + bit[i^mask];
    }
  }
