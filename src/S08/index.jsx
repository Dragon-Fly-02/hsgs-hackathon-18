...
class Board extends React.Component {
  render() {
    ...
    for (let i = 1; i <= h; i++)
      if (col[i] === "white")
        moves.push(
          <div>
            <button
              className="buttoncircle1"
              onClick={() => this.props.move({ x: i })}
            >
              {i}
            </button>
          </div>
    ...
    return (
      <div className="example s08">
        <p>
          Có {h} quả bóng, {(h - (h % 2)) / 2} trắng, còn lại đen. Hãy xếp chúng
          vào cột sao cho:<br />
          +Các bóng lẻ kề nhau.<br />
          +Các bóng trắng kề nhau.<br />
          +Số trái cùng gấp đôi số gần trái cùng.<br />
        </p>
        <button className="khung" onClick={this.props.StartNewGame}>
          {" "}
